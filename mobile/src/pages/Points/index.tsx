  ...
  return (
    <>
      <View style={styles.container}>
        ...
        <Text style={styles.description}>Encontre no mapa ponto de coleta.</Text>

        <View style={styles.mapContainer}>
          { initPosition[0] !== 0 && (
            <MapView style={styles.map} loadingEnabled={initPosition[0] === 0} initialRegion={{ latitude: initPosition[0], longitude: initPosition[1], latitudeDelta:0.014, longitudeDelta:0.014}}>
              {points.map(point => (
      ...
      <View style={styles.itemsContainer}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20}}>
          {items.map(item => (
            <TouchableOpacity key={String(item.id)} style={[styles.item, selectedItems.includes(item.id) ? styles.selectedItem : {}]} onPress={() => {handelerSelectItem(item.id)}} activeOpacity={0.6}>
